...
whiteWins, blackWins = getTrain(N_INPUT, TOTAL_MLP, VOLUME_SIZE)	#, True)
...
print 'shuffling...'
start = time.time()
x_train = whiteWins + blackWins
idx = range(len(x_train))
random.shuffle(idx)
x_train = x_train[idx]
print 'elapsed: %.2f sec' % (time.time() - start)

validation_split = int(len(x_train) * 0.1)
x_test = x_train[-validation_split:]
x_train = x_train[:-validation_split]

autoencoder.fit(x_train, x_train, nb_epoch=50,	# 'epochs' in the later versions
